...
export default class ArticleTable {
  ...
  static upgradeTable(db: Database.Database) {
    ...
    if (!tableInfo.includes("count")) {
      const alertStmt = db.prepare(
        "ALTER TABLE articles ADD COLUMN count INTEGER DEFAULT 0",
      );
      alertStmt.run();
  ...
  static getAllArticles(db: Database.Database) {
    // 按照创建时间倒序
    const stmt = db.prepare(
      "SELECT * FROM articles WHERE is_delete = 0 ORDER BY create_time DESC",
    );
  ...
  static updateArticle(
    ...
    const documentItemStmt = db.prepare(
      "UPDATE document_items SET update_time = ?, content = ?, count = ? WHERE is_article = 1 AND article_id = ?",
    );
    documentItemStmt.run(now, JSON.stringify(content), count, id);

    // update project_item
    const projectItemStmt = db.prepare(
      "UPDATE project_item SET update_time = ?, content = ?, count = ? WHERE ref_type = 'article' AND ref_id = ?",
    );
