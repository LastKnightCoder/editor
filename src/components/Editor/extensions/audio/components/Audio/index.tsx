...
const Audio = (props: IExtensionBaseProps<AudioElement>) => {
  ...
  const [textToSpeechTransformLoading, setTextToSpeechTransformLoading] = useState(false);
  ...
  const handleNetworkUrlChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleTextToSpeechContentChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const handleOnTextToSpeechInputFinish = async () => {
    ...
    try {
      const audio = await textToSpeech(appid, token, textToSpeechValue, currentSpeakerId);
      if (!audio) {
  ...
  const applyGeneratedAudio = async () => {
    ...
    const file = new File([byteArray], `${Date.now()}.mp3`, { type: 'audio/mp3' });
  ...
  const renderNoContent = () => {
    return (
      <Empty
        description={'暂未设置音频，请上传'}
      >
        <Flex vertical gap={12} justify={'center'}>
          <Flex align={'center'} gap={12} justify={'center'}>
            <Button disabled={readOnly} onClick={() => uploadRef.current?.click()}>本地上传</Button>
            <Popover
              ...
              content={(
                <Flex gap={12}>
                  <Input size={'large'} style={{ width: 500 }} value={src} onChange={handleNetworkUrlChange} />
                ...
                <Flex vertical gap={12} style={{ minWidth: 400 }}>
                  ...
                  <If condition={!!textToSpeechAudio}>
                    <Flex gap={12} align={'center'}>
                      <audio controls src={`data:audio/mp3;base64,${textToSpeechAudio}`} />
