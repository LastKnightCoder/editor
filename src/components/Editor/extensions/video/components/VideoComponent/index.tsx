...
const VideoComponent = (props: IExtensionBaseProps<VideoElement>) => {
  ...
  const handleNetworkUrlChange = useMemoizedFn((event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleUploadFileChange = useMemoizedFn(async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    ...
    >
      <Spin spinning={uploading}>
        ...
        <If condition={!!src}>
          <LocalVideo width={'100%'} controls src={src} playbackRate={playbackRate} />
        </If>
        <div {...attributes}>
          {children}
        </div>
      ...
      <input ref={uploadRef} type={'file'} accept={'video/*'} hidden onChange={handleUploadFileChange}/>
