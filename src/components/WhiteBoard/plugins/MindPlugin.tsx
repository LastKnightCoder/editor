...
export class MindPlugin implements IBoardPlugin {
  ...
  isElementSelected(board: Board, element: MindNodeElement, selectArea: Selection['selectArea'] = board.selection.selectArea) {
    if (!selectArea) return false;
    const selectRect = selectAreaToRect(selectArea);
    return isRectIntersect(element, selectRect);
  }

  render(_board: Board, { element, children }: { element: MindNodeElement, children?: any }) {
    return (
      <g key={element.id}>
