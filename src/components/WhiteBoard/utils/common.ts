...
export const getResizedBBox = (
  ...
  if (
    [EHandlerPosition.Left, EHandlerPosition.Right].includes(position) &&
    isPreserveRatio
  ) {
    newHeight = newWidth / ratio;
  } else if (
    [EHandlerPosition.Bottom, EHandlerPosition.Top].includes(position) &&
    isPreserveRatio
  ) {
