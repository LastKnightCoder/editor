...
const ArrowActivePoint = memo((props: ArrowActivePointProps) => {
  ...
  const { run: handlePointerMove } = useThrottleFn(
    (e: PointerEvent) => {
      ...
      const updateArrowElement = getUpdateArrowElement(
        lastArrowElement.current,
        currentPoint.current,
        index,
      );
      if (!pathRef.current) {
        pathRef.current = PathUtil.getPathByElement(
          board,
          lastArrowElement.current,
        );
      }
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (e: PointerEvent) => {
      ...
      lastArrowElement.current = PathUtil.getElementByPath(
        board,
        path,
      ) as ArrowElement;
  ...
  return (
    <g ref={ref}>
      <circle cx={point.x} cy={point.y} r={outerSize} fill={outerFill} />
      <circle cx={point.x} cy={point.y} r={innerSize} fill={innerFill} />
    </g>
  );
});
