...
const useCreateGeometry = () => {
  ...
  const handlePointerUp = useMemoizedFn((_e: PointerEvent) => {
    if (startPoint.current && currentPoint.current && isMoved.current && createdGeometry.current && createdGeometryPath.current) {
      board.apply([{
        type: 'remove_node',
        path: createdGeometryPath.current,
  ...
  }, [createBoardElementType, handlePointerDown, handlePointerMove, handlePointerUp])
}
