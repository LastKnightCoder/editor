...
const AliOssSetting = () => {
  ...
  const { accessKeyId = '', accessKeySecret = '', bucket = '', region= '' } = setting;
  ...
  return (
    <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
      ...
      <Space>
        ...
        <Space>
          <Input.Password
            ...
            onChange={(e) => {
              useSettingStore.setState(produce((state) => {
                state.setting.imageBed.aliOSS.accessKeySecret = e.target.value;
              }));
            }}
            allowClear
          />
      ...
      <If condition={bucketsInfo.length > 0 || !!bucket && !!region}>
        <Space>
          <div>Bucket：</div>
          <Select style={{ width: 400 }} options={bucketsInfo.map(bucketInfo => ({ label: bucketInfo.bucket, value: bucketInfo.bucket }))} onChange={onSelectBucket} value={bucket} />
        </Space>
      </If>
    </Space>
  )
