...
const useCardConfig = () => {
  ...
  const filteredCards = useMemo(() => {
    ...
    const filteredCards = cards.filter(card => {
      const isHitCategory = categoryFilterArray.length === 0 || categoryFilterArray.includes(card.category);
      if (!isHitCategory) return false;
      const matchedVecDocuments = vecDocuments.filter(vecDocument => vecDocument.refId === card.id);
      let status;
  ...
  const slicedCards = filteredCards.slice((current - 1) * PAGE_SIZE, current * PAGE_SIZE);
            ...
            <>
              <Button
                ...
                onClick={async () => {
                  ...
                  await onUpdateEmbedding(markdown, record).then(() => {
                    message.success({
                      key: 'updateEmbedding',
                      content: '更新嵌入成功',
                      duration: 2,
