...
const useDragAndDrop = (item: IDragAndDropParams) => {
  ...
  const [{ isOver, canDrop }, drop] = useDrop<IDragItem, IDragItem, {
    ...
    canDrop: (dragItem) => {
      ...
      if (dragPosition === EDragPosition.Inside && (dragItem.parentId === itemId || children.includes(dragItem.itemId))) {
        return false;
      }
      const isSameLevel = dragPath.length === dropPath.length && JSON.stringify(dragPath.slice(0, -1)) === JSON.stringify(dropPath.slice(0, -1));
      if (dragPosition !== EDragPosition.Inside && parentChildren.includes(dragItem.itemId) && !isSameLevel) {
        return false;
      }
