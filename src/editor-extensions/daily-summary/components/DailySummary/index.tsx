...
const DailySummary = (props: IExtensionBaseProps<DailySummaryElement>) => {
  ...
  const cardsWithDate = cards.filter(
    (card) =>
      dayjs(card.create_time).format("YYYY-MM-DD") === date ||
      dayjs(card.update_time).format("YYYY-MM-DD") === date,
  );
  const articlesWithDate = articles.filter(
    (article) =>
      dayjs(article.create_time).format("YYYY-MM-DD") === date ||
      dayjs(article.update_time).format("YYYY-MM-DD") === date,
  );
