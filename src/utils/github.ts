...
export const getBranchesByRepo = async (token: string, owner: string, repo: string) => {
...
export const getPathAndContentFromFile = async (file: File): Promise<{
  path: string;
  content: string;
} | null> => {
...
export const uploadFileFromContent = async (githubInfo: IGithubInfo, commitInfo: ICommitInfo) => {
  ...
  const res = await axios.put(`/repos/${owner}/${repo}/contents/${path}`, {
    branch,
    message,
    content,
    committer,
