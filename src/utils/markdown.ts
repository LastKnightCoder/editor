...
const markdownToDescendant = (node: any, parent: any): Descendant | Descendant[] | null => {
  if (node.type === 'heading') {
    ...
    return node.children.map(markdownToDescendant).filter(Boolean).map((item: Descendant | Descendant[]) => {
...
export const importFromMarkdown = (markdown: string): Descendant[] => {
  ...
  const dfs = (children: any[], result: Descendant[], parent = null) => {
    for (const child of children) {
      while (child.type === 'link' && child.children && child.children.some((c: any) => c.type === 'html')) {
        const index = child.children.findIndex((child: any) => child.type === 'html');
  ...
  let count = 10;
  while (JSON.stringify(beforeNormalize) !== JSON.stringify(afterNormalize) && count > 0) {
