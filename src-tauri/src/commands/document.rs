...
#[tauri::command]
pub fn create_document_item(title: String, authors: Vec<String>, tags: Vec<String>, is_directory: bool, children: Vec<i64>, is_article: bool, article_id: i64, is_card: bool, card_id: i64, content: String, banner_bg: String, icon: String, parents: Vec<i64>, app_state: tauri::State<state::AppState>) -> Result<i64, String> {
    let conn = app_state.db.lock().unwrap();
    match &*conn {
        Some(conn) => {
            let res = document::create_document_item(&conn, &title, authors, tags, is_directory, children, is_article, article_id, is_card, card_id, &content, &banner_bg, &icon, parents).map_err(|e| e.to_string())?;
            Ok(res)
        }
        None => Err("Database connection not initialized".to_string()),
    }
...
#[tauri::command]
pub fn update_document_item(id: i64, title: String, authors: Vec<String>, tags: Vec<String>, is_directory: bool, children: Vec<i64>, is_article: bool, article_id: i64, is_card: bool, card_id: i64, content: String, banner_bg: String, icon: String, parents: Vec<i64>, app_state: tauri::State<state::AppState>) -> Result<document::DocumentItem, String> {
    let conn = app_state.db.lock().unwrap();
    match &*conn {
        Some(conn) => {
            document::update_document_item(&conn, id, &title, authors, tags, is_directory, children, is_article, article_id, is_card, card_id, &content, &banner_bg, &icon, parents).map_err(|e| e.to_string())
        }
        None => Err("Database connection not initialized".to_string()),
    }
}
...
#[tauri::command]
pub fn get_all_document_items(app_state: tauri::State<state::AppState>) -> Result<Vec<document::DocumentItem>, String> {
    let conn = app_state.db.lock().unwrap();
    match &*conn {
        Some(conn) => {
            document::get_all_document_items(&conn).map_err(|e| e.to_string())
        }
        None => Err("Database connection not initialized".to_string()),
    }
}
...
#[tauri::command]
pub fn init_all_document_item_parents(app_state: tauri::State<state::AppState>) -> Result<(), String> {
    let conn = app_state.db.lock().unwrap();
    match &*conn {
        Some(conn) => {
            document::init_all_document_item_parents(&conn).map_err(|e| e.to_string())
        }
        None => Err("Database connection not initialized".to_string()),
    }
}
...
#[tauri::command]
pub fn init_document_item_parents_by_ids(ids: Vec<i64>, app_state: tauri::State<state::AppState>) -> Result<(), String> {
    let conn = app_state.db.lock().unwrap();
    match &*conn {
        Some(conn) => {
            document::init_document_item_parents_by_ids(&conn, ids).map_err(|e| e.to_string())
        }
        None => Err("Database connection not initialized".to_string()),
    }
}
...
#[tauri::command]
pub fn get_document_item_all_parents(id: i64, app_state: tauri::State<state::AppState>) -> Result<Vec<i64>, String> {
    let conn = app_state.db.lock().unwrap();
    match &*conn {
        Some(conn) => {
            document::get_document_item_all_parents(&conn, id).map_err(|e| e.to_string())
        }
        None => Err("Database connection not initialized".to_string()),
    }
}